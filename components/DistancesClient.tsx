...
const NUMERIC_COLS = new Set(['net_weight_kg', 'distance_km', 'transport1_amount_zar', 'transport2_amount_zar', 'payout'])
            ...
            {loading ? (
              ...
              rows.map((row, i) => (
                <tr key={i} className="border-b border-slate-100 transition-colors last:border-0 hover:bg-slate-50/70">
                  ...
                  <td className="whitespace-nowrap px-3 py-1.5 text-right tabular-nums font-semibold text-slate-900">{formatZar(getPayout(row))}</td>
                </tr>
              ))
            )}
          </tbody>
